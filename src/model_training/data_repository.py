...
class DataRepository:
    def __init__(self, env_path: str):
        ...
        self.DIRTY_DATA_APPROACH1_FILE = os.getenv("DIRTY_DATA_APPROACH1_FILE")
    ...
    def load_clean_file_names(self):
        #Read xlsx file
        train_files_df = pd.read_excel(self.CLEAN_DATA_APPROACH1_FILE, sheet_name="train_dataset")
        validation_files_df = pd.read_excel(self.CLEAN_DATA_APPROACH1_FILE, sheet_name="validation_dataset")
        test_files_df = pd.read_excel(self.CLEAN_DATA_APPROACH1_FILE, sheet_name="test_dataset")
        #get the files names
        train_files = train_files_df['files'].values.tolist()
        validation_files = validation_files_df['files'].values.tolist()
        test_files = test_files_df['files'].values.tolist()
    ...
    def load_dirty_current_data(self):
        print("Loading dirty current data ...")
        file_name = self.DIRTY_DATA_APPROACH1_FILE
        train_files_df = pd.read_excel(file_name, sheet_name="train_dataset")
        validation_files_df = pd.read_excel(file_name, sheet_name="validation_dataset")
        test_files_df = pd.read_excel(file_name, sheet_name="test_dataset")
        train_files = train_files_df['files'].values.tolist()
    ...
    def read_train_current_survey_data(self, file_path):
        X_trains, y_trains = [], []
        for i in range(0, 10):
            sheet_name = "Train_dataset_"+str(i)
            train_files_df = pd.read_excel(file_path, sheet_name=sheet_name)
            train_files = train_files_df['files'].values.tolist()
            X_train, y_train = self.import_current_data_from_file_list(train_files)
    ...
    def read_train_fft_survey_data(self, file_path):
        X_trains, y_trains = [], []
        for i in range(0, 10):
            sheet_name = "Train_dataset_"+str(i)
            train_files_df = pd.read_excel(file_path, sheet_name=sheet_name)
    ...
    def read_test_current_survey_data(self, file_path):
        X_tests, y_tests = [], []
        for i in range(0, 10):
            sheet_name = "Test_dataset_"+str(i)
            test_files_df = pd.read_excel(file_path, sheet_name=sheet_name)
            test_files = test_files_df['files'].values.tolist()
            X_test, y_test = self.import_current_data_from_file_list(test_files)
            X_tests.append(X_test)
            y_tests.append(y_test)
        return np.array(X_tests), np.array(y_tests)
    
    def read_test_fft_survey_data(self, file_path):
        X_tests, y_tests = [], []
        for i in range(0, 10):
            sheet_name = "Test_dataset_"+str(i)
            test_files_df = pd.read_excel(file_path, sheet_name=sheet_name)
